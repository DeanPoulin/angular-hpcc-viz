(function () {
  ...
  google.setOnLoadCallback(function () {
    angular.element(document).ready(function () {
      angular.bootstrap(document, ['viz.demos']);
    });
  });

  angular.module('viz.demos', ['angular.hpcc.viz']).controller('VizCtrl', function ($scope, viz) {

    // Table and charts
    $scope.chartModes = viz.chart.ChartModes;
    $scope.chartMode = $scope.chartModes[0];

    $scope.chartTypes = viz.chart.ChartTypes;
    $scope.chartType = $scope.chartTypes[0];

    $scope.chartColumns = ['Subject', 'Year 1', 'Year 2', 'Year 3'];
    $scope.chartData = [
      ['Geography', 75, 68, 65],
      ['English', 45, 55, 52],
      ['Math', 98, 92, 90],
    ...
    $scope.clickOnMultiChartGraphPoint = function (data, columnName) {
      console.log('multichartsurface-point-click', data, columnName);
    };


    // Wordcloud
    $scope.wordcloudColumns = ['Word', 'Weight'];
    $scope.wordcloudData = ["Myriel", "Napoleon", "Mlle.Baptistine", "Mme.Magloire", "CountessdeLo", "Geborand", "Champtercier", "Cravatte", "Count", "OldMan", "Labarre", "Valjean", "Marguerite", "Mme.deR", "Isabeau", "Gervais", "Tholomyes", "Listolier", "Fameuil", "Blacheville", "Favourite", "Dahlia", "Zephine", "Fantine", "Mme.Thenardier", "Thenardier", "Cosette", "Javert", "Fauchelevent", "Bamatabois", "Perpetue", "Simplice", "Scaufflaire", "Woman1", "Judge", "Champmathieu", "Brevet", "Chenildieu", "Cochepaille", "Pontmercy", "Boulatruelle", "Eponine", "Anzelma", "Woman2", "MotherInnocent", "Gribier", "Jondrette", "Mme.Burgon", "Gavroche", "Gillenormand", "Magnon", "Mlle.Gillenormand", "Mme.Pontmercy", "Mlle.Vaubois", "Lt.Gillenormand", "Marius", "BaronessT", "Mabeuf", "Enjolras", "Combeferre", "Prouvaire", "Feuilly", "Courfeyrac", "Bahorel", "Bossuet", "Joly", "Grantaire", "MotherPlutarch", "Gueulemer", "Babet", "Claquesous", "Montparnasse", "Toussaint", "Child1", "Child2", "Brujon", "Mme.Hucheloup"].map(function (d) { return [ d, 10 + Math.random() * 14 ]; });


    // Morphtext
    var alphabet = 'visualization'.split('');
    $scope.morphtext = alphabet.join('');
    setInterval(function () {
      $scope.morphtext = shuffle(alphabet).join('');
      $scope.$digest();
    }, 2000);


    // Choropleth: countries
    var countriesCodeToName = viz.map.Countries.countryNames;
    var countriesNameToCode = Object.keys(countriesCodeToName).reduce(function (m, code) {
      m[countriesCodeToName[code].name] = code;
      return m;
    }, {});
    var countriesData = [
      {name: 'United States', weight: 29.946185501741 },
      {name: 'China', weight: 229.946185501741 }
    ];
    $scope.countriesColumns = ['Countries', 'Weight', 'Label'];
    $scope.countriesData = countriesData.map(function (item) {
      return {
        label: item.name,
        weight: item.weight,
        country: countriesNameToCode[item.name]
      };
    });


    // Choropleth: states
    var statesCodeToName = viz.map.USStates.stateNames;
    var statesNameToCode = Object.keys(statesCodeToName).reduce(function (m, code) {
      m[statesCodeToName[code].name] = statesCodeToName[code].code;
      return m;
    }, {});
    var statesData = [
    ...
    $scope.statesColumns = ['States', 'Weight', 'Label'];
    $scope.statesData = statesData.map(function (item) {
      return [statesNameToCode[item.name], item.weight, item.name];
    });
    ...
    $scope.countiesColumns = ["Counties", "Weight", "Label"];
    $scope.countiesData = countiesData.map(function (item) {
    ...
    addresses.forEach(function (item) {
        var newAddr = new viz.common.Shape()
            .shape('circle')
            .radius(3)
            .data(item);
        vertices.push(newAddr);
        if (prevAddr) {
            edges.push(new viz.graph.Edge()
    ...
    $scope.gmapData = {vertices: vertices, edges: edges};
      ...
      children: [
        { label: "A",
          children: [
            ...
            {label: "AB", children: [{label: "ABA"}]}
          ]
        },
        { label: "B",
          children: [
            {label: "BA", children: [{label: "BAA"}]},
            {label: "BB"}
          ]
        }
      ]
    };
    $scope.treeData1 = cloneDeep(treeData);
    $scope.treeData2 = cloneDeep(treeData);
    $scope.treeData3 = cloneDeep(treeData);


    // Slider
    $scope.sliderValueChanged = function (val) {
      console.log('Slider value: ', val);
    };
    $scope.sliderRangeChanged = function (start, end) {
      console.log('Slider range: ', start, end);
    };


    // Surface
    var surfaceContent = new viz.map.ChoroplethStates().testData().paletteID("BuPu");
    $scope.surfaceMenu = viz.common.Palette.rainbow();
    $scope.surfaceContent = surfaceContent;
    $scope.surfaceMenuClicked = function (selection) {
      surfaceContent.paletteID(selection).render();
    };


    // Grid
    $scope.gridContents = [
      [0, 0, new viz.common.Shape().shape("circle").size({ width: 32, height: 32 }), 'Circle', 1, 1],
      [0, 1, new viz.common.FAChar().char("\uf080"), 'Icon', 1, 1],
      [1, 0, new viz.tree.Dendrogram().testData(), 'Dendrogram', 1, 1],
      [1, 1, new viz.tree.CirclePacking().testData(), 'Circle Packing', 1, 1]
    ];


    // Graph
    var vertices = [
      new viz.common.Shape().shape("circle").size({ width: 32, height: 32 }),
      new viz.common.FAChar().char("\uf080"),
      new viz.common.Icon().shape("circle").faChar("\uf080"),
      new viz.common.Shape().class("Shape2").shape("rect").size({ width: 32, height: 32 }),
      new viz.common.Text().text("Multi\nLine\nText"),
      new viz.common.TextBox().text("Text\nBox"),
      new viz.graph.Vertex().text("Graph\nVertex").faChar("\uf080"),
      new viz.common.Shape().shape("rect").size({ width: 48, height: 22 }),
      new viz.common.List().testData(),
      new viz.common.Menu().faChar("\uf0c9").data(["Menu A", "And B", "a longer C"]),
      new viz.common.ResizeSurface()
        .size({ width: 200, height: 100 })
        .title("Resize Me!!!")
        .menu(["aaa", "bbb", "ccc"])
        .icon_faChar("\uf047")
        .content(new viz.tree.Dendrogram().testData())
    ];
    var edges = [
      createEdge(vertices[0], vertices[2]),
      createEdge(vertices[1], vertices[2]),
      createEdge(vertices[3], vertices[5]),
      createEdge(vertices[4], vertices[5]),
      createEdge(vertices[2], vertices[6]),
      createEdge(vertices[5], vertices[6], "test label"),
      createEdge(vertices[7], vertices[9]),
      createEdge(vertices[1], vertices[9]),
      createEdge(vertices[8], vertices[9]),
      createEdge(vertices[6], vertices[10]),
      createEdge(vertices[7], vertices[10]),
      createEdge(vertices[9], vertices[10])
    ];

    $scope.graphLayouts = viz.graph.LayoutTypes;
    $scope.graphLayout = $scope.graphLayouts[0];
    $scope.graphData = {vertices: vertices, edges: edges};
    $scope.graphVertexClick = function (vertex, event) {
      console.log('vertex-click', vertex, event);
    };

    function createEdge(source, target, label) {
      return new viz.graph.Edge()
        .sourceVertex(source)
        .targetVertex(target)
    ...
    $scope.persistState = '{ "__version": 3, "__class": "c3chart_Bar", "__id": "_w51", "__properties": {"isStacked": true, "paletteID": "Dark2" } }';
    $scope.persistColumns = $scope.chartColumns;
    $scope.persistData = $scope.chartData;
  });
